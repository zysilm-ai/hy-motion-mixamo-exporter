...
def _animateRotationKeyFrames(animLayer, node, rot_matrices, frameDuration):
    ...
    for nth in range(len(rot_matrices)):
        ...
        # Ensure float64 and contiguous array for NumPy 2.0 compatibility with transforms3d
        rot_mat = np.array(rot_matrices[nth], dtype=np.float64, copy=True)
        euler = np.rad2deg(mat2euler(rot_mat, axes="sxyz"))
